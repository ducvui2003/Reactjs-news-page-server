...
app.get("/", cors(corsOptions), async (req, res, next) => {
    ...
    if (!category) {
        const errorXml = createErrorXml("Category query parameter is required", 400);
        return res
            .status(400).send(errorXml)
    }
    ...
    try {
        ...
        const errorXml = createErrorXml("An error occurred while fetching the RSS feed.", 500);
